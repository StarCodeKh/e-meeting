        ...
        <main class="meeting-viewport container-fluid px-4">
            ...
            <div v-else class="card-stack d-flex flex-column gap-3 pb-5">
                <div v-for="(m, index) in sortedMeetings" :key="index" 
                     class="meeting-card row mx-0 align-items-center shadow-sm py-3 rounded-3 bg-light" 
                    ...
                    <div class="col-12 col-lg-8 border-start-lg ps-lg-4 mb-3 mb-lg-0">
                        ...
                        <div class="meta-info khmer-font d-flex flex-wrap gap-4">
                            ...
                            <div class="d-flex align-items-center text-muted small">
                                ...
                                <span>ដឹកនាំដោយ៖ <b class="text-dark">{{ m.participantsDisplay || m.leader }}</b></span>
                            </div>
                        </div>
                    </div>

                    <div class="col-12 col-lg-2">
                        <div class="status-box d-flex align-items-center justify-content-center gap-2 py-3 rounded-3 fw-bold shadow-sm" 
...
<script setup>
    ...
    const calculateStatus = (start, end) => {
        ...
        const toMin = (t) => { if(!t || t==='--:--') return null; const [h,m] = t.split(':').map(Number); return h*60+m }
        const sMin = toMin(start); const eMin = toMin(end) || (sMin + 60)
