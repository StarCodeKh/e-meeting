...
<script setup>
    ...
    // ១. កំណត់ Tabs & Colors ជា Constant (Standard)
    const TABS = [
    ...
    // ២. Reactive Form (Dynamic Initialization)
    const form = reactive({
        type: 'meeting', 
        title: '', 
    ...
    // ៣. Computed Properties (Dynamic Styles)
    ...
    // ៤. Handle Save (Standard Payload & Validation)
    const handleSave = async () => {
        // Validation បឋម (Frontend Standard)
        if (!form.title.trim()) return alertStore.show('សូមបញ្ចូលចំណងជើង', 'warning');
        if (form.start_time >= form.end_time) return alertStore.show('ម៉ោងបញ្ចប់ត្រូវធំជាងម៉ោងចាប់ផ្តើម', 'warning');

        loading.value = true
        try {
            const payload = {
                ...form,
                // បំប្លែង String ទៅ Array សម្រាប់ JSON column ក្នុង Laravel
                participants: form.participants.split(',').map(p => p.trim()).filter(p => p !== ""),
                // ប្តូរឈ្មោះ key ឱ្យត្រូវនឹង Migration: $table->string('color_id')
                color_id: form.color 
            }
            ...
            emit('refresh')
            closeModal()
            resetForm() // សម្អាត Form ក្រោយជោគជ័យ
        } catch (err) {
    ...
    const resetForm = () => {
        Object.assign(form, {
            title: '', participants: '', location: '', room: '', color: 'green'
        })
    }
</script>
